...
#include <string.h>
#include <stdint.h>
#include <asm/unistd.h>         // _NR_##syscall

char *text = "Test text";

static void increment2(int var) {
    printf("\nIncrement a value using a register =====\n");

    printf("value before increment: %d\n", var);

    asm("incl %0"
        : "+a" (var)
        : // the input will be the same as output: governed by the + sign in the output specifier (here, eax)
        : "cc");   // arithmetic instructions generally modify flags, 
                   // tell gcc to notice this fact (cc - condition codes)" 
