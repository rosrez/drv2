...
static enum hrtimer_restart hrt_timerfunc(struct hrtimer *timer)
{
    ...
    if (info->count < shots) {
        ...
        /* increase the period twice and reschedule the timer */
        if (info->count == shots) {
            info->period = ktime_set(0, 2 * ns);
            /* kt is 'now' so the period + now gives us the target time */
            hrtimer_forward(timer, kt, info->period);
        }
